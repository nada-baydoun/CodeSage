...
export class Logger {
  ...
  private formatMessage(level: string, message: string, data?: unknown) {
    ...
    const base: { prefix: string; message: string; data?: unknown } = {
      prefix,
      message,
    };
    if (typeof data !== "undefined") base.data = data;
    return base;
  }
  ...
  error(message: string, error?: Error | unknown, data?: unknown) {
    ...
    const formattedData = this.formatMessage("error", message, payload);
